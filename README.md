...
a stderr the emulator does not write into (see "Running on arm64"); CI replays every one of them on
an amd64 kernel, so a verdict-affecting change is not verified until CI is green. The fixtures
themselves are captured on that same runner (`npm run capture:e2e`, the `capture-e2e` workflow job)
and reviewed by a human before they are committed.

The kill ladder in `src/verdict` is covered a third way: `test/fixtures/measurements` holds real
sandbox measurements captured inside the image (`dist/tools/captureMeasurements.js`), and the unit
suite replays each through `gradeCase`. CI re-captures and diffs them whenever the sandbox, the
verdict module or the fixtures change, and nightly. The `derived/` ones are generated by
`test/tools/deriveMeasurements.ts` (`--check` in CI). See the `verdicts-and-comparison` skill.
