...
/**
 * Something a transcript depends on that QEMU user-mode emulation on an
 * arm64 host (where `/health` reports `seccomp: "disabled"`) cannot give:
 *
 *  - `rlimit_as` — `RLIMIT_AS` is silently not enforced, so MLE cannot fire;
 *  - `seccomp` — the amd64 BPF program cannot be installed, so nothing is
 *    filtered;
 *  - `native` — a byte-clean stderr. The emulator writes its own
 *    diagnostics into the guest's stderr: `qemu: uncaught target signal 11
 *    (Segmentation fault) - core dumped` when a program dies by a signal,
 *    and PyPy prints `Warning: cannot find your CPU L2 cache size in
 *    /proc/cpuinfo` against the emulated `/proc`. On a real kernel both
 *    streams are empty, and that emptiness is what the transcript pins.
 */
export type Requirement = "rlimit_as" | "seccomp" | "native";
...
export const SCENARIOS: readonly Scenario[] = [
  ...
  },
  {
    ...
    // The traceback names the per-submission workdir: only the exception
    // is stable, exactly as for `re-python-exception`.
    patterns: { "results[0].stderr": "MemoryError" },
    measured: { cpuMs: ">0" },
  },
  {
